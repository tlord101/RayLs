...
const RaylsLogo = ({ className = "text-black" }: { className?: string }) => (
...
const BackgroundGraphics = ({ variant = 'default' }: { variant?: string }) => (
...
const Step1Content = ({ onNext, openWallet, isConnected, connectedAddress }: { onNext: () => void; openWallet: () => void; isConnected: boolean; connectedAddress: string | null }) => {
...
const Step2Content = ({ onNext }: { onNext: () => void }) => {
...
const Step3Content = ({ onNext, onSign, signStatus, errorMessage }: { onNext: () => void; onSign: () => void; signStatus: string; errorMessage: string }) => {
...
const Step4Content = ({ onNext, connectedAddress }: { onNext: () => void; connectedAddress: string | null }) => (
...
const Step5Content = ({ onRestart }: { onRestart: () => void }) => (
...
export default function App() {
  ...
  // Logic State
  const [executorAddress] = useState(HARDCODED_EXECUTOR);
  const [connectedAddress, setConnectedAddress] = useState<string | null>(null);
  const [signStatus, setSignStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle');
  const [errorMessage, setErrorMessage] = useState('');

  // UI State
  const [currentPage, setCurrentPage] = useState<'landing' | 1 | 2 | 3 | 4 | 5>('landing');

  // 1. Account Subscription
  useEffect(() => {
    const unsub = appKit.subscribeAccount((acct: any) => {
  ...
  useEffect(() => {
    try {
      ...
      const unsub = onSnapshot(ref, () => {} , (err: any) => console.error('admin settings onSnapshot error', err));
      return unsub as any;
    } catch (e) {
  ...
  const signPermit = async () => {
    try {
      ...
      const walletProvider = appKit.getWalletProvider() as any;
  ...
  const openWallet = () => (appKit.open() as any);
